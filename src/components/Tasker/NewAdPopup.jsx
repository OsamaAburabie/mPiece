...
export default function Popup({ disable, taskerId, success, handleShow }) {
  ...
  async function postImage({ image, title, location, desc, price }) {
    try {
      ...
      const result = await axios.post(
        `http://localhost:5000/api/newpost/${catId}`,
        formData,
        {
          headers: {
  ...
  const book = async () => {
    if (title && price && desc && location && typeof numPrice === "number") {
      setDesabled(true);
    }
    await postImage({
      image: file,
